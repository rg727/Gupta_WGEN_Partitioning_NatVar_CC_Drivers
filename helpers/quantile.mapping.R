quantile.mapping <- function(prcp.site,S,prct.prob,perc.q,emission.old1,emission.old2,emission.new1,n.sites,months,dates.sim,jitter) {
  ...
  #prct.prob = threshold for gpd fit
  #n.sites = the number of individual sites
  #months = a vector of calendar months for the season
  #dates.sim = a time series of the simulated dates
  #jitter = TRUE/FALSE indicating whether to perturb the non-exceedance probabilities
  ...
  prcp.site.u1.new <- prcp.site.u1
  
  if (jitter) {   # if we want to perturb U's, jump into Z space to do the jittering :)
    #then get Zs
    prcp.site.z1 <- lapply(prcp.site.u1,function(x) {qnorm(x)})
    #simulate new Zs
    jitter.samp <- rmvnorm(n,rep(0,n.sites),S.cond)
    jitter.samp1 <- sapply(1:n.sites,function(x) {jitter.samp[idx1[[x]],x]})
    
    prcp.site.z1.proposed <- sapply(1:n.sites,function(x) {prcp.site.z1[[x]]+jitter.samp1[[x]]})

    #transform new Z's to new U's
    prcp.site.u1.proposed <- lapply(prcp.site.z1.proposed,function(x){pnorm(x)})

    #accept proposed u with probability proportional to ratio of old and new exceedance ratios
    coin.flip <- sapply(1:n.sites,function(x) {runif(length(prcp.site.u1[[x]]))})
    
    prcp.site.u1.new <- sapply(1:n.sites,function(i,x,y,cf) {
      final <- y[[i]]
      ...
      return(final)
    },x=prcp.site.u1.proposed,y=prcp.site.u1,cf=coin.flip)
  }
  #replace any probability==1 (to avoid getting 'inf' instances in next 'sapply') with a very close value to 1.
  check.probs.one <- sapply(1:n.sites,function(x) {sum(prcp.site.u1.new[[x]]==1)>0})
  if (sum(check.probs.one==1)>0){
    idx.ones <- lapply(prcp.site.u1.new,function(x){which(x==1)})
    prcp.site.u0.new <- sapply(1:n.sites,function(x) {
      replace(prcp.site.u1.new[[x]],idx.ones[[x]],1-.Machine$double.eps)
    })
    prcp.site.u1.new <- prcp.site.u0.new
  }
  
  #transform new U's to new precipitation using new emission distributions
  prcp.site.new1 <- sapply(1:n.sites,function(x,z,m,mm) {
    qgamma(prcp.site.u1.new[[x]],'shape'=z[1,match(m[[x]],mm),x],'rate'=z[2,match(m[[x]],mm),x])
  ...
  if (jitter) {   # if we want to perturb U's, jump into Z space to do the jittering :)
    ...
    #accept proposed u with probability proportional to ratio of old and new exceedance ratios
    coin.flip <- sapply(1:n.sites,function(x) {runif(length(prcp.site.u2[[x]]))})

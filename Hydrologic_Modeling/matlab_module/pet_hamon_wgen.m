function pet = pet_hamon(S_Date, E_Date, Tavg, Latitude, Coeff,j,i,sim_datemat)
%#codegen
...
%P=table(sim_datemat,daylighthr_mat);
%filename = sprintf('testdata_%d.xlsx',i);
%filename = sprintf('E:/sacsma_calfews_cornell/sacsma_calfews_cornell/pet_ensemble_fixed/%d/PET_%d.txt',j,i);
%writetable(P,filename,'Delimiter','\t')
%pet = 0.611*exp(17.27*Tavg./(237.3+Tavg));
%pet  = Coeff*29.8*daylighthr_mat.*(esat./(Tavg+273.2));
%test = load(['E:\sacsma_calfews_cornell\sacsma_calfews_cornell\pet_ensemble_fixed\',num2str(j),'\PET_filtered',num2str(i),'.txt']); % yr mon day pr(mm) tas(C)
%daylighthr_mat=test(:,4);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
